...
### Fixed
- Prevented unbounded growth in pending message buffers by adding TTL-based cleanup for inactive users.

### Added
- Added `CHAT_BUDDY_PENDING_REPLY_TTL_HOURS` and `CHAT_BUDDY_PENDING_REPLY_CLEANUP_INTERVAL_MS` for pending reply cleanup tuning.
